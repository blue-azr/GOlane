/*
 * Copyright © 2008-2023 Audinate Pty Ltd ACN 120 828 006 (Audinate). All rights reserved.
 *
 *
...
/**
 * Sets the client's keepalive interval - 3s by default.
 *
 * @param config the config object
 * @param interval_s the frequency (in seconds) of keepalive packets
 *
 * @note Only invoke this function if you have proficient understanding.
 */
void
conmon_client_config_set_keepalive_interval
(
	conmon_client_config_t* config,
	uint16_t interval_s
);
...
/**
 * Sets the client's keepalive timeout - 10s by default.
 *
 * @param config the config object
 * @param timeout_s the number of seconds without receiving a message before the client is considered disconnected
 *
 * @note Only invoke this function if you have proficient understanding.
 */
void
conmon_client_config_set_keepalive_timeout
(
	conmon_client_config_t* config,
	uint16_t timeout_s
);
 ...
 * @note This function is only valid if the client is currently connected.
 *
 * @note at present, the only valid non-metering monitoring channels are
 *   CONMON_CHANNEL_TYPE_STATUS and CONMON_CHANNEL_TYPE_TELEMETRY.
 */
aud_error_t
