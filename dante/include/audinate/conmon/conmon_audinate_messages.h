...
enum conmon_audinate_message_version
{
	...
	*/
	...
	/**
	 * - Added CONMON_AUDINATE_CAPABILITY1_CAN_GET_TIME_FROM_RX capability
	 */
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_43 = 0x0700 + 43,

	/**
	* - Added video control & status message types,
	*/
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_44 = 0x0700 + 44,

	/**
	* - Added following capabilities to the versions status message
	* - CONMON_AUDINATE_CAPABILITY1_HAS_RDONLY_PREFERRED_MASTER in capabilities1
	* - CONMON_AUDINATE_CAPABILITY1_HAS_RDONLY_SLAVE_ONLY in capabilities1
	*/
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_45 = 0x0700 + 45,

	/**
	 * - Added CONMON_AUDINATE_CLOCKING_FLAG_PREFERRED_LOCKED flag to clocking status
	 * - Added 8-byte/Ptp-v2 based Master clock ID and GrandMaster clock ID to clocking status
	 */
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_46 = 0x0700 + 46,
	
	/**
	 * - Added CONMON_AUDINATE_CAPABILITY1_SINGLE_ADDRESS_RX_MCAST capability
	 */
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_47 = 0x0700 + 47,

	/**
	 * - Added CONMON_AUDINATE_DEVICE_STATUS_FLAG_FPGA_TYPE_AND_CAP_PARTS_MISMATCH_ERROR
	 * - Added CONMON_AUDINATE_CAPABILITY1_VIDEO_TX
	 * - Added CONMON_AUDINATE_CAPABILITY1_VIDEO_RX
	 * - Added CONMON_AUDINATE_CAPABILITY1_CAN_TX_AUDIO_DELAY
	 */
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_48 = 0x0700 + 48,

	/**
	 * - Added web plugin IDs to the versions message
	 */
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_49 = 0x0700 + 49,

	/**
	 * - Added UUID to DDM enrolment status message
	 */
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_50 = 0x0700 + 50,

	/**
	* - Added advert change notification messages for transmit channels and multicast
	* transmit flows
	*/
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_51 = 0x0700 + 51,

	/**
	* Added device display flags in versions status message.
	*/
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_52 = 0x0700 + 52,

	/**
	* Added device uuid in versions status message.
	*/
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_53 = 0x0700 + 53,

	/**
	* Added CAPABILITY1_HAS_ARGO_BOOTSTRAP
	* Added Argo Addresses control message
	*/
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_54 = 0x0700 + 54,

	/**
	* Added Argo Addresses status message
	*/
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_55 = 0x0700 + 55,

	/**
	* Added OEM readonly control, query and status messages
	* Added read-only status flag for External word clock
	*/
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_56 = 0x0700 + 56,

	/**
	 * Extended clock control and status with additional fields.
	 */
	CONMON_AUDINATE_MESSAGE_VERSION_0_7_57 = 0x0700 + 57,

	/** The current version */
	CONMON_AUDINATE_MESSAGE_VERSION = CONMON_AUDINATE_MESSAGE_VERSION_0_7_57
};
...
enum conmon_audinate_message_type
{
	...
	* Clocking control messages allow configuration of a Dante device's
	...
	/**
	* TX channel advert change messages provide asynchronous notification that
	* the advertised data for a device's transmit channel has changed. Only the id's
	* of changed channels are provided; detailed information about the channels
	* is available via the routing API.
	*
	* Format: conmon_audinate_id_set_msg
	* Channel: Status
	*/
	CONMON_AUDINATE_MESSAGE_TYPE_TX_CHANNEL_ADVERT_CHANGE = 0x010c,

	/**
	* TX flow advert change messages provide asynchronous notification that the
	* advertised data for a device's multicast transmit flow has changed. Only
	* the id's of changed flows are provided; detailed information about the
	* flows is available via the routing API.
	*
	* Format: conmon_audinate_id_set_msg
	* Channel: Status
	*/
	CONMON_AUDINATE_MESSAGE_TYPE_TX_FLOW_ADVERT_CHANGE = 0x010d,
	...
	*/
	...
	/**
	 * Query/control a device's video parameters
	 *
	 * Format: conmon_audinate_video_control_t
	 * Channel: Control
	 */
	CONMON_AUDINATE_MESSAGE_TYPE_VIDEO_CONTROL = 0x100D,

	/**
	 * Status response from device containing video information
	 *
	 * Format: conmon_audinate_video_status_t
	 * Channel: Status
	 */
	CONMON_AUDINATE_MESSAGE_TYPE_VIDEO_STATUS = 0x100E,

	/**
	 * Control message to device with support for bootstrapping argo connection
	 *
	 * Format:  conmon_audinate_argo_bootstrap_control_t
	 * Channel: Control
	 */
	CONMON_AUDINATE_MESSAGE_TYPE_ARGO_BOOTSTRAP_CONTROL = 0x100F,

	/**
	 * Status response from device with support for bootstrapping argo connection
	 *
	 * Format: conmon_audinate_argo_bootstrap_status_t
	 * Channel: Status
	 */
	CONMON_AUDINATE_MESSAGE_TYPE_ARGO_BOOTSTRAP_STATUS = 0x1010,

	/**
	* Reserved for oem readonly mode for a Dante device
	*
	* Format: conmon_audinate_oem_readonly_control
	* Channel: Control
	*/
	CONMON_AUDINATE_MESSAGE_TYPE_OEM_READONLY_CONTROL = 0x1021,

	/**
	* Query oem readonly mode for a Dante device
	*
	* Format: query
	* Channel: Control
	*/
	CONMON_AUDINATE_MESSAGE_TYPE_OEM_READONLY_QUERY  = 0x1022,

	/**
	* Status of oem readonly mode (enabled/disabled) for a Dante device
	*
	* Format: conmon_audinate_oem_readonly_status
	* Channel: Status
	*/
	CONMON_AUDINATE_MESSAGE_TYPE_OEM_READONLY_STATUS = 0x1023,
};
...
/**
 * How many elements (bytes) of the audio media type ids are there in this message?
 *
 * This method is unsafe and does not check the length of the message.
 *
 * @deprecated applications should use conmon_audinate_id_set__for_media_type
 * with a media type of DANTE_MEDIA__AUDIO and the conmon_audinate_id_set_media_record__num_elements function instead.
 *
...
/**
 * Get the index'th element of the audio media type ids in this message
 *
 * This method is unsafe and does not check the length of the message.
 *
 * @deprecated applications should use conmon_audinate_id_set__for_media_type
 * with a media type of DANTE_MEDIA__AUDIO and the conmon_audinate_id_set_media_record__element_at_index function instead.
 *
 * @param aud_msg a conmon message body containing an Audinate id set message
...
// ID set media extension

/**
 * Helper structure for reading media idsets
 *
 * Do not access these fields directly
 */
typedef struct conmon_audinate_id_set_media_read
{
	const conmon_message_body_t * aud_msg;
	size_t len;
	const struct conmon_audinate_id_set_msg_media * media;
	const uint16_t * records;
} conmon_audinate_id_set_media_read_t;

/**
 * Get the pointer to the media section of the message
 *
 * @param aud_msg a conmon message body containing an Audinate id set message
 * @param len the length of the conmon message body
 * @param out_media_read uninitialised media read structure
 *
 * @return AUD_ERR_SUCCESS if media read successfully initialised with media info
 *	AUD_ERR_DONE if no media info
 *  error otherwise
 */
aud_error_t
conmon_audinate_id_set__get_media_only
(
	const conmon_message_body_t* aud_msg,
	const size_t len,
	conmon_audinate_id_set_media_read_t * out_media_read
);


/**
 * Get pointer to media section of the message, or the legacy audio section
 *
 * As conmon_audinate_id_set__get_media_only(), but will return a special
 * form of out_media_read to support legacy audio-only messages.
 */
aud_error_t
conmon_audinate_id_set__get_media
(
	const conmon_message_body_t* aud_msg,
	const size_t len,
	conmon_audinate_id_set_media_read_t * out_media_read
...
/**
 * Get the number of media records present in the message
 *
 * @param media_read reference to media section
 */
uint16_t
conmon_audinate_id_set__num_media_records
(
	const conmon_audinate_id_set_media_read_t * media_read
);


/**
 * Helper structure for reading a single idset media record
 *
 * Do not access these fields directly
 */
typedef struct conmon_audinate_id_set_media_record_read
{
	const conmon_audinate_id_set_media_read_t * parent;
	const struct conmon_audinate_id_set_media_record * record;
	const uint8_t * bytes;
} conmon_audinate_id_set_media_record_read_t;


/**
 * Get a pointer to the index'th media record in the message
 *
 * @param media_read initialised media_read structure
 * @param index the media record index
 * @param out_record_read uninitialised record_read structure
 */
const aud_error_t
conmon_audinate_id_set__media_record_at
(
	const conmon_audinate_id_set_media_read_t * media_read,
	const uint16_t index,
	conmon_audinate_id_set_media_record_read_t * out_record_read
);

/**
 * Gets the media type of the media record
 *
 * @param media_record reference to the media record in the message
 */
dante_media_type_t
conmon_audinate_id_set_media_record__media_type
(
	const conmon_audinate_id_set_media_record_read_t * media_record
);

/**
 * Gets the number of elements (bytes) in the set of ids in a media record
 *
 * @param media_record reference to the media record in the message
 */
uint16_t
conmon_audinate_id_set_media_record__num_elements
(
	const conmon_audinate_id_set_media_record_read_t * media_record
);

/**
 * Get the index'th element of the set of ids in a media record
 *
 * @param media_record reference to the media record in the message
 * @param index the element index
 */
uint8_t
conmon_audinate_id_set_media_record__element_at_index
(
	const conmon_audinate_id_set_media_record_read_t * media_record,
	uint16_t index
);
...
#define CONMON_AUDINATE_NUM_CLOCK_EXT_CAPABILITIES 6
...
enum conmon_v2_port_sync_interval
{
	CONMON_AUDINATE_V2_SYNC_INTERVAL_MAX		= 0,
	CONMON_AUDINATE_V2_SYNC_INTERVAL_MIN		= -7
};

enum conmon_v2_port_announce_interval
{
	CONMON_AUDINATE_V2_ANNOUNCE_INTERVAL_MAX		= 1,
	CONMON_AUDINATE_V2_ANNOUNCE_INTERVAL_MIN		= -3,
};

enum conmon_v2_port_delay_request_interval
{
	CONMON_AUDINATE_V2_DELAY_REUQEST_INTERVAL_MAX	= 0,
	CONMON_AUDINATE_V2_DELAY_REUQEST_INTERVAL_MIN	= -4,
};

enum conmon_preferred_protocol
{
	CONMON_AUDINATE_PREFERRED_PROTOCOL_PTPV1		= 1,	// ptp v1 protocol preferred when running both
	CONMON_AUDINATE_PREFERRED_PROTOCOL_PTPV2		= 2		// ptp v2 protocol preferred when running both
};

enum conmon_ptp_priority_mapping
{
	CONMON_AUDINATE_PTP_PRIORITY_MAPPING_1		= 1,	// revision 1 - used for AES67 mode interop
	CONMON_AUDINATE_PTP_PRIORITY_MAPPING_2		= 2	// revision 2
};

enum conmon_v2_port_delay_mechanism
{
	CONMON_AUDINATE_V2_PORT_DELAY_MECHANISM_E2E	= 1,
	CONMON_AUDINATE_V2_PORT_DELAY_MECHANISM_PEER = 2
};

// TTL value for multicast ptp port
enum conmon_ptp_port_ttl
{
	CONMON_AUDINATE_PTP_PORT_TTL_MIN			= 1,
	CONMON_AUDINATE_PTP_PORT_TTL_MAX			= 64,
};
...
/**
 * A type representation for PTP V2 sync interval value
 */
typedef int8_t conmon_audinate_clock_v2_sync_interval_t;

/**
 * A type representation for PTP V2 annouce interval value
 */
typedef int8_t conmon_audinate_clock_v2_announcec_interval_t;

/**
 * A type representation for PTP V2 delay request interval value
 */
typedef int8_t conmon_audinate_clock_v2_delay_request_interval_t;

/**
 * A type representation for PTP V2 peer delay intervale value
 */
typedef int8_t conmon_audinate_clock_v2_peer_delay_interval_t;

/**
 * A type representation for PTP V2 delay_mechanism value
 */
typedef uint8_t conmon_audinate_clock_v2_delay_mechanism_t;

/**
 * A type representation for PTP ttl value
 */
typedef uint8_t conmon_audinate_clock_ttl_t;

/**
 * A type representation for PTP V2 priority
 */
typedef uint8_t conmon_audinate_clock_v2_priority_t;

/**
 * A type representation for PTP priority mapping
 */
typedef uint8_t conmon_audinate_clock_priority_mapping_t;

/**
 * A type representation for PTP preferred protocol
 */
typedef uint8_t conmon_audinate_clock_preferred_protocol_t;

/**
 * A type representation for the additional capabilities for a clocking status
 *
 * @since Audinate message version 0.7.57
 */
typedef uint32_t conmon_audinate_clock_ext_capabilities_t;
 ...
 * @deprecated use conmon_audinate_v2_clock_uuid_t
 */
typedef struct
{
	  /** the UUID itself */
...
/**
 * IEEE1588-2008 based length of an Audinate clock ID
 */
#define CONMON_AUDINATE_V2_CLOCK_UUID_LENGTH 8

/**
 * IEEE1588 and IEEE1588-2008 devices are identified by a 8-byte UUID
 */
typedef struct
{
	  /** the UUID itself */
	uint8_t data[CONMON_AUDINATE_V2_CLOCK_UUID_LENGTH];
} conmon_audinate_v2_clock_uuid_t;
...
/**
 * Clocking status extension
 * @since Audinate message version 0.7.46
 * Can Ptp v1 'Preferred' flag be configured?
 */
aud_bool_t
conmon_audinate_clocking_status_is_preferred_locked
(
	const conmon_message_body_t * aud_msg
);
 ...
 * Clocking status extension
 * @since Audinate message version 0.7.46
 * Get IEEE1588-2008 (v2) based device's clock id
 */
const conmon_audinate_v2_clock_uuid_t *
conmon_audinate_clocking_status_get_v2_uuid
(
	const conmon_message_body_t * aud_msg
);
 ...
 * Clocking status extension
 * @since Audinate message version 0.7.46
 * Get IEEE1588-2008 (v2) based master clock id
 */
const conmon_audinate_v2_clock_uuid_t *
conmon_audinate_clocking_status_get_v2_master_uuid
(
	const conmon_message_body_t * aud_msg
);
 ...
 * Clocking status extension
 * @since Audinate message version 0.7.46
 * Get IEEE1588-2008 (v2) based grandmaster clock id
 */
const conmon_audinate_v2_clock_uuid_t *
conmon_audinate_clocking_status_get_v2_grandmaster_uuid
(
	const conmon_message_body_t * aud_msg
);
...
/**
 * Clocking status extension
 * @since Audinate message version 0.7.56
 * Can "external word clock" be configured?
 */
aud_bool_t
conmon_audinate_clocking_status_is_external_wc_locked
(
	const conmon_message_body_t* aud_msg
);
...
/**
 * @since Audinate message version 0.7.57
 * Is Dante-ness of PTP priorities enabled?
 */
aud_error_t
conmon_audinate_clocking_status_is_danteness_enabled
(
	const conmon_message_body_t * aud_msg,
	aud_bool_t * enabled
);
...
/**
 * @since Audinate message version 0.7.57
 * Get ptp v2 priority 1
 */
aud_error_t conmon_audinate_clocking_status_get_v2_priority1
(
	const conmon_message_body_t * aud_msg,
	uint8_t * priority
);
...
/**
 * @since Audinate message version 0.7.57
 * Get ptp v2 priority 2
 */
aud_error_t conmon_audinate_clocking_status_get_v2_priority2
(
	const conmon_message_body_t * aud_msg,
	uint8_t * priority
);

// Clock extended capaibility flags
enum conmon_clock_ext_capability_flags
{
	// from conmon version 0.7.57
	CONMON_AUDINATE_CLOCK_EXT_CAPABILITY_FLAG__CAN_SWITCH_PTP_PROTOCOL = (1 << 0),
	CONMON_AUDINATE_CLOCK_EXT_CAPABILITY_FLAG__CAN_SET_PTP_V2_PRIORITIES = (1 << 1),
	CONMON_AUDINATE_CLOCK_EXT_CAPABILITY_FLAG__CAN_SET_PTP_V2_DOMAIN_NUMBER = (1 << 2),
	CONMON_AUDINATE_CLOCK_EXT_CAPABILITY_FLAG__CAN_SWITCH_PTP_V2_DELAY_MECHANISM = (1 << 3),
	CONMON_AUDINATE_CLOCK_EXT_CAPABILITY_FLAG__CAN_SET_PTP_V2_MESSAGE_INTERVALS = (1 << 4),
	CONMON_AUDINATE_CLOCK_EXT_CAPABILITY_FLAG__CAN_SET_PORT_TTL = (1 << 5)
};

/**
 * @since Audinate message version 0.7.57
 * Get extended clock capability flags. See enum conmon_clock_ext_capability_flags
 */
conmon_audinate_clock_ext_capabilities_t
conmon_audinate_clocking_status_get_ext_capabilities
(
	const conmon_message_body_t * aud_msg
);
...
/**
 * @since Audinate message version 0.7.57
 * Get ptp v2 sync interval value of a given port
 * @param[in] aud_msg Pointer to the message boday that contains clock status
 * @param[out] sync_interval sync interval value pointer
 * @param[in] ps status extention 3 pointer
 */
aud_error_t
conmon_audinate_clocking_status_get_v2_port_sync_interval
(
	const conmon_message_body_t * aud_msg,
...
/**
 * @since Audinate message version 0.7.57
 * Get ptp v2 announce interval value of a given port
 * @param[in] aud_msg Pointer to the message boday that contains clock status
 * @param[out] announce_interval announce interval value pointer
 * @param[in] ps status extention 3 pointer
 */
aud_error_t
conmon_audinate_clocking_status_get_v2_port_announce_interval
(
	const conmon_message_body_t * aud_msg,
...
/**
 * @since Audinate message version 0.7.57
 * Get ptp v2 delay request interval value of a given port
 * @param[in] aud_msg Pointer to the message boday that contains clock status
 * @param[out] delay_request_interval delay request interval value pointer
 * @param[in] ps status extention 3 pointer
 */
aud_error_t
conmon_audinate_clocking_status_get_v2_port_delay_request_interval
(
	const conmon_message_body_t * aud_msg,
	int8_t *delay_request_interval,
	const conmon_audinate_port_status_ext3_t * ps
);

/**
 * @since Audinate message version 0.7.57
 * Get ptp v2 peer delay interval value of a given port
 * @param[in] aud_msg Pointer to the message boday that contains clock status
 * @param[out] peer_delay_interval peer delay interval value pointer
 * @param[in] ps status extention 3 pointer
 */
aud_error_t
conmon_audinate_clocking_status_get_v2_port_peer_delay_interval
(
	const conmon_message_body_t * aud_msg,
	int8_t *peer_delay_interval,
	const conmon_audinate_port_status_ext3_t * ps
);

/**
 * @since Audinate message version 0.7.57
 * Get ptp v2 delay mechanism value of a given port
 * @param[in] aud_msg Pointer to the message boday that contains clock status
 * @param[out] delay_mechanism delay mechanism value pointer
 * @param[in] ps status extention 3 pointer
 */
aud_error_t
conmon_audinate_clocking_status_get_v2_port_delay_mechanism
(
	const conmon_message_body_t * aud_msg,
...
/**
 * @since Audinate message version 0.7.57
 * Get ptp slave only value of a given port
 * @param[in] aud_msg Pointer to the message boday that contains clock status
 * @param[out] slave_only slave only value pointer
 * @param[in] ps status extention 3 pointer
 */
aud_error_t
conmon_audinate_clocking_status_get_port_slave_only
(
	const conmon_message_body_t * aud_msg,
	aud_bool_t *slave_only,
	const conmon_audinate_port_status_ext3_t * ps
);

/**
 * @since Audinate message version 0.7.57
 * Get TTL value of a given multicast ptp port
 * @param[in] aud_msg Pointer to the message boday that contains clock status
 * @param[out] slave_only ttl value pointer
 * @param[in] ps status extention 3 pointer
 */
aud_error_t conmon_audinate_clocking_status_get_port_ttl
(
	const conmon_message_body_t * aud_msg,
	uint8_t * ttl,
	const conmon_audinate_port_status_ext3_t * ps
...
/**
 * @since Audinate message version 0.7.57
 * Get ptp v2 clock class
 * @param[in] aud_msg Pointer to the message boday that contains clock status
 * @param[out] clock_class clock class value pointer
 */
aud_error_t conmon_audinate_clocking_status_get_v2_clock_class
(
	const conmon_message_body_t * aud_msg,
	uint8_t * clock_class
);
...
/**
 * @since Audinate message version 0.7.57
 * Which ptp protocol is the preferred protocol?
 * @param[in] aud_msg Pointer to the message boday that contains clock status
 * @param[out] preferred_protocol preferred protocol pointer
 */
aud_error_t conmon_audinate_clocking_status_get_preferred_protocol
(
	const conmon_message_body_t * aud_msg,
...
/**
 * @since Audinate message version 0.7.57
 * Which mappings are used between ptp v1 and ptp v2 priorities.
 * @param[in] aud_msg Pointer to the message boday that contains clock status
 * @param[out] priority_mapping priority mapping pointer
 */
aud_error_t conmon_audinate_clocking_status_get_priority_mapping
(
	const conmon_message_body_t * aud_msg,
	uint8_t * priority_mapping
);
...
enum conmon_capability_index
{
	...
	CONMON_AUDINATE_CAPABILITY_INDEX_HAS_NO_REMOTE_LOCK_RESET = 28, // Note the negation!
...
{
	CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_GET_DATA = 0,
	...
	CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_GET_TIME_FROM_RX = 7,
	CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_VCP  = 8,
	CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_RDONLY_PREFERRED_MASTER = 9,
	CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_RDONLY_SLAVE_ONLY = 10,
	CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_DDM_SUPPORT_CAPABILITIES = 11,
	CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_DDM_ENROL_DEVICE = 12,
	CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_DDM_ASSOCIATE_DEVICE = 13,
	CONMON_AUDINATE_CAPABILITY1_INDEX_SINGLE_ADDRESS_RX_MCAST = 14,
	CONMON_AUDINATE_CAPABILITY1_INDEX_VIDEO_TX = 15,
	CONMON_AUDINATE_CAPABILITY1_INDEX_VIDEO_RX = 16,
	CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_TX_AUDIO_DELAY = 17,
	CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_NETWORK_ACTIVATION = 18,
	CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_WEBSOCKET = 19,
	CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_CLEAR_CONFIG_WITHOUT_REBOOT = 20,
	CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_ARGO_BOOTSTRAP = 21,
	CONMON_AUDINATE_NUM_CAPABILITIES1
};

// legacy compatibility
#define CONMON_AUDINATE_CAPABILITY_INDEX_CAN_GET_DATA CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_GET_DATA

enum conmon_capability1
{
	CONMON_AUDINATE_CAPABILITY1_CAN_GET_DATA					= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_GET_DATA),
	...
	CONMON_AUDINATE_CAPABILITY1_CAN_GET_TIME_FROM_RX						= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_GET_TIME_FROM_RX),
	CONMON_AUDINATE_CAPABILITY1_HAS_VCP										= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_VCP),
	CONMON_AUDINATE_CAPABILITY1_HAS_RDONLY_PREFERRED_MASTER 				= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_RDONLY_PREFERRED_MASTER),
	CONMON_AUDINATE_CAPABILITY1_HAS_RDONLY_SLAVE_ONLY 						= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_RDONLY_SLAVE_ONLY),
	CONMON_AUDINATE_CAPABILITY1_HAS_DDM_SUPPORT_CAPABILITIES 				= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_DDM_SUPPORT_CAPABILITIES),
	CONMON_AUDINATE_CAPABILITY1_CAN_DDM_ENROL_DEVICE 						= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_DDM_ENROL_DEVICE),
	CONMON_AUDINATE_CAPABILITY1_CAN_DDM_ASSOCIATE_DEVICE 					= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_DDM_ASSOCIATE_DEVICE),
	CONMON_AUDINATE_CAPABILITY1_SINGLE_ADDRESS_RX_MCAST						= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_SINGLE_ADDRESS_RX_MCAST),

	CONMON_AUDINATE_CAPABILITY1_VIDEO_TX									= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_VIDEO_TX),
	CONMON_AUDINATE_CAPABILITY1_VIDEO_RX									= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_VIDEO_RX),
	CONMON_AUDINATE_CAPABILITY1_CAN_TX_AUDIO_DELAY							= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_TX_AUDIO_DELAY),
	CONMON_AUDINATE_CAPABILITY1_HAS_NETWORK_ACTIVATION						= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_NETWORK_ACTIVATION),
	CONMON_AUDINATE_CAPABILITY1_HAS_WEBSOCKET								= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_WEBSOCKET),
	CONMON_AUDINATE_CAPABILITY1_CAN_CLEAR_CONFIG_WITHOUT_REBOOT				= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_CAN_CLEAR_CONFIG_WITHOUT_REBOOT),
	CONMON_AUDINATE_CAPABILITY1_HAS_ARGO_BOOTSTRAP							= (1 << CONMON_AUDINATE_CAPABILITY1_INDEX_HAS_ARGO_BOOTSTRAP),
};

// legacy compatibility
#define CONMON_AUDINATE_CAPABILITY_CAN_GET_DATA CONMON_AUDINATE_CAPABILITY1_CAN_GET_DATA
...
enum
{
	...
	CONMON_AUDINATE_DEVICE_STATUS_INDEX_FPGA_TYPE_AND_CAP_PARTS_MISMATCH_ERROR,
...
enum
{
	...
	/** There is a mismatch between the capability and FPGA type, e.g. FPGA is TX and capability is for RX */
	CONMON_AUDINATE_DEVICE_STATUS_FLAG_FPGA_TYPE_AND_CAP_PARTS_MISMATCH_ERROR
		= (1 << CONMON_AUDINATE_DEVICE_STATUS_INDEX_FPGA_TYPE_AND_CAP_PARTS_MISMATCH_ERROR),
};
...
#define CONMON_WEB_PUGIN_ID_STR_MAX_LEN 23

/**
 * Data type to represent a web plugin ID in a null terminated string format
 * 
 */
typedef struct
{
	char d[CONMON_WEB_PUGIN_ID_STR_MAX_LEN + 1]; // Extra one byte for null terminating character
} conmon_web_plugin_id_str_t;

/**
 * Gets the number of web plugin IDs in the conmon versions status message
 * 
 * @param aud_msg a conmon message body containing a conmon versions status message
 * @return uint16_t the number of web plugin ids in this conmon versions status message
 */
uint16_t
conmon_audinate_versions_status_get_num_web_plugin_ids
(
	const conmon_message_body_t * aud_msg
);

/**
 * Adds a web plugin ID to the conmon versions status message 
 * 
 * @param aud_msg a conmon message body containing a conmon versions status message
 * @param size a structure that contains info about the size of the conmon versions status message
 * @param plugin_id the null terminated string representation of a web plugin ID, @see conmon_web_plugin_id_str_t
 * @return aud_error_t AUD_SUCCESS if the plugin ID was added successfully else AUD_ERR*
 */
aud_error_t
conmon_audinate_versions_status_add_web_plugin_id
(
	conmon_message_body_t * aud_msg,
	conmon_message_size_info_t * size,
	conmon_web_plugin_id_str_t * plugin_id
);

/**
 * Returns a single web plugin ID. An index has to be passed into retrieve the web plugin ID.
 * Call @see conmon_audinate_versions_status_get_num_web_plugin_ids to get the 
 * number of web plugin IDs 
 * 
 * @param aud_msg a conmon message body containing a conmon versions status message
 * @param web_plugin_id_idx zero based index into the array of web plugin IDs
 * @param out_web_plugin_id_str the web plugin ID returned from this function
 * @return aud_error_t AUD_SUCCESS if the plugin ID was added successfully else AUD_ERR*
 */
aud_error_t
conmon_audinate_versions_status_get_web_plugin_id
(
	const conmon_message_body_t * aud_msg,
	uint16_t const web_plugin_id_idx,
	conmon_web_plugin_id_str_t ** out_web_plugin_id_str
);

/**
 * Get the uuid from the versions status message
 *
 * @param aud_msg a conmon message body containing an audinate versions message
 */
const char *
conmon_audinate_versions_status_get_uuid
(
	const conmon_message_body_t * aud_msg
);
...
enum
{
	...
	CONMON_AUDINATE_UPGRADE_PROTOCOL_HTTP = 4,
		//!< http url path
	CONMON_AUDINATE_UPGRADE_PROTOCOL_HTTPS = 5,
		//!< https url path

	CONMON_AUDINATE_UPGRADE_PROTOCOL_COUNT = 5
};
...
/**
	Get bit-mask marking known actions.

	@param aud_msg Initialised upgrade control message
 */
conmon_audinate_upgrade_control_actions_t
conmon_audinate_upgrade_control_message_get_actions_valid
(
	const conmon_message_body_t * aud_msg
);
...
/**
	...
	@param aud_msg Initialised upgrade control message
	@param manufacturer The override manufacturer ID field
	@param model The override model ID field
 */
aud_error_t
...
/**
	Get information about the manufacturer and/or model override flags

	@param aud_msg Initialised upgrade control message
	@param manufacturer The override manufacturer flag
	@param model The override model flag
 */
aud_error_t
conmon_audinate_upgrade_control_get_override_flags
(
	const conmon_message_body_t * aud_msg,
	aud_bool_t * manufacturer,
	aud_bool_t * model
);
...
/**
 * Set the board rev field in the edk board status message
 *
 * @param aud_msg a conmon message body containing an edk board status message
 * @param rev the board rev value
 */
void
conmon_audinate_edk_board_status_set_rev
(
	conmon_message_body_t * aud_msg,
	uint16_t rev
);
...
/**
 * Set the board pad field in the edk board status message
 *
 * @param aud_msg a conmon message body containing an edk board status message
 * @param pad the board pad value
 */
void
conmon_audinate_edk_board_status_set_pad
(
	conmon_message_body_t * aud_msg,
	uint16_t pad
);
...
/**
 * Set the board dig field in the edk board status message
 *
 * @param aud_msg a conmon message body containing an edk board status message
 * @param dig the board dig value
 */
void
conmon_audinate_edk_board_status_set_dig
(
	conmon_message_body_t * aud_msg,
	uint16_t dig
);
...
/**
 * Set the board src field in the edk board status message
 *
 * @param aud_msg a conmon message body containing an edk board status message
 * @param src the board src value
 */
void
conmon_audinate_edk_board_status_set_src
(
	conmon_message_body_t * aud_msg,
	uint16_t src
);
...
/**
 * Get the size of an edk board status message
 *
 * @param aud_msg a conmon message body containing an edk board status message
 */
uint16_t
conmon_audinate_edk_board_status_get_size
(
	const conmon_message_body_t * aud_msg
);
...
uint16_t
conmon_audinate_aes67_status_get_size
(
	const conmon_message_body_t * aud_msg
);
...
/**
 * Initialise a video control message but do not set any of the fields.
 *
 * @param aud_msg A conmon message body that will hold the message
 * @param congestion_delay_window_us the recommended maximum delay before sending a response
 */
void
conmon_audinate_init_video_control
(
	conmon_message_body_t * aud_msg,
	uint32_t congestion_delay_window_us
);
...
/**
 * Set the video control payload into the conmon message body
 *
 * @param aud_msg A conmon message body containing an video control message
 * @param payload A raw pointer to the video control payload
 * @param payload_size Size of the the video control payload
 * @return AUD_SUCCESS On success, otherwise an error reason.
 */
aud_error_t
conmon_audinate_video_control_set_payload
(
	conmon_message_body_t * aud_msg,
	uint8_t *payload,
...
/**
 * Get the raw memory pointer to the video control payload.
 *
 * @param aud_msg A conmon message body containing an video control message
 * @return A raw pointer to the video control payload.
 */
void *
conmon_audinate_video_control_get_payload
(
	const conmon_message_body_t * aud_msg
);

/**
 * Get the size of the payload for the video control packet
 *
 * @param aud_msg A conmon message body containing an video control message
 * @return Size of the payload in bytes
 */
uint16_t
conmon_audinate_video_control_get_payload_size
(
	const conmon_message_body_t * aud_msg
);

/**
 * Get the size (in bytes) of the video control message
 *
 * @param aud_msg A conmon message body containing an video control message
 * @return Size of the video control message in bytes
 */
uint16_t
conmon_audinate_video_control_get_size
(
	const conmon_message_body_t * aud_msg
);

/**
 * Initialise a video status message but do not set any of the fields.
 *
 * @param aud_msg A conmon message body that will hold the message
 * @param congestion_delay_window_us the recommended maximum delay before sending a response
 */
void
conmon_audinate_init_video_status
(
	conmon_message_body_t * aud_msg,
	uint32_t congestion_delay_window_us
);
...
/**
 * Set the video status payload into the conmon message body
 *
 * @param aud_msg A conmon message body containing an video status message
 * @param payload A raw pointer to the video status payload
 * @param payload_size Size of the the video status payload
 * @return AUD_SUCCESS On success, otherwise an error reason.
 */
aud_error_t
conmon_audinate_video_status_set_payload
(
	conmon_message_body_t * aud_msg,
	void * payload,
	uint16_t payload_size
...
/**
 * Get the size (in bytes) of the video status message
 *
 * @param aud_msg A conmon message body containing an video status message
 * @return Size of the video status message in bytes
 */
uint16_t
conmon_audinate_video_status_get_size
(
	const conmon_message_body_t * aud_msg
);

/**
 * Gets the raw memory pointer to the video status payload.
 *
 * @param aud_msg A conmon message body containing an video status message
 * @return Raw memory pointer to the video status payload.
 */
void *
conmon_audinate_video_status_get_payload
(
	const conmon_message_body_t * aud_msg
);

/**
 * Gets the size of the payload for the video status packet
 *
 * @param aud_msg A conmon message body containing an video status message
 * @return Size of the payload in bytes
 */
uint16_t
conmon_audinate_video_status_get_payload_size
(
	const conmon_message_body_t * aud_msg
);
...
/**
 * Is the read-only flag enabled on the device?
 * If this function returns true, then some parameters on the device can't
 * be configured. e.g., device name, sample rate etc. The affected parameters
 * are marked as read-only in the relevant APIs.
 *
 * @param aud_msg A conmon message body containing read-only status message
 * @return State of the read-only flag
 */

aud_bool_t
conmon_audinate_oem_readonly_status_is_enabled
(
	const conmon_message_body_t * aud_msg
);
