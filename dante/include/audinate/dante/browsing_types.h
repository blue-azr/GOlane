...
typedef enum
{
	...
	DB_NODE_CHANGE_CONFIRMED
	// An existing node was queried but did not change.
		// Only happens when a device component has been re-resolved.
} db_node_change_t;
...
/**
 * When called within a db_browse_node_changed_fn callback, get the types of information
 * that triggered a db_node_change_t event for this device.
 * If the function returns '0' then the device changed callback has already been fired
 * and the variable has been cleared thereafter.
 * 
 * @param device device node
 */
db_browse_types_t
db_browse_device_get_changed_browse_type
(
	const db_browse_device_t* device
);
...
/**
 * Get the fully qualified mDNS name for this device.
 *
 * @param device device node
 */
const char*
db_browse_device_get_fqdn
(
	const db_browse_device_t* device
); 
 ...
 * Get the dante instance id for this device (if known).
 * This value is only known if
 * the upgrade mode advertisement for this device has been discovered
 * That is, the DB_BROWSE_TYPE_UPGRADE_MODE_DEVICE bit must be set when calling
 * db_browse_device_get_browse_types is called for this device.
 *
 * @param device device node
 */
const dante_instance_id_t *
db_browse_device_get_upgrade_mode_instance_id
(
	const db_browse_device_t * device
);
...
(
	db_browse_device_t * device,
	db_browse_types_t browse_types, // if 0 then reconfirm all known browses for this device
...
/**
* Return browse types with in progress resolves.
*
* @param device device node
* @return bitmap of browse types still being resolved
*
* @note This is only applicable when browsing in the adhoc domain. Zero in progress resolves is always returned if the browse is not browsing in the adhoc domain.
*/
db_browse_types_t
db_browse_device_get_resolves_inprogress
(
	const db_browse_device_t * device
);

/**
* Returns whether there are browse types with in progress resolves.
*
* @param device device node
* @return AUD_TRUE if there are browse types with in progress resolves. AUD_FALSE otherwise.
*
* @note This is only applicable when browsing in the adhoc domain. AUD_FALSE is always returned if the browse is not browsing in the adhoc domain.
*/
aud_bool_t
db_browse_device_has_resolves_inprogress
(
	const db_browse_device_t * device
);
