...
typedef enum
{
	...
	DR_DEVICE_COMPONENT_PROPERTIES,
		/** The device's display groups */
	DR_DEVICE_COMPONENT_DISPLAYGROUPS,
...
enum
{
	...
	DR_DEVICE_CHANGE_FLAG_PROPERTIES = (1 << DR_DEVICE_COMPONENT_PROPERTIES),
		/** The device's display groups  */
	DR_DEVICE_CHANGE_FLAG_DISPLAYGROUPS = (1 << DR_DEVICE_COMPONENT_DISPLAYGROUPS),
...
typedef enum
{
	...
	DR_DEVICE_STATE_QUERYING,
		/** The device's capabilities are known and more information
		    has been requested */
 ...
 * asynchronous action requires a 'request'. These requests are shared
 ...
 * asynchronous action requires a 'request'. These requests are
...
/**
 * Set whether pending requests fire their callbacks on device close.
 *
 * When a dr_device is closed or becomes invalid, all in-flight requests are
 * silently aborted. Set AUD_TRUE to instead cause in-flight requests to abort
 * and then fire their callbacks with AUD_ERR_CLOSING.
 *
 * @param devices        [IN] the devices object
 * @param fire_callbacks [IN] the indicator to opt-in to fire callbacks or not
 */
void
dr_devices_set_fire_callbacks_on_device_close
(
	dr_devices_t * devices,
	aud_bool_t fire_callbacks
);

/**
 * Get whether pending requests fire their callbacks on device close.
 *
 * @param devices        [IN] the devices object
 * @param fire_callbacks [OUT] the value of the indicator to opt-in to fire callbacks or not
 *
 * @note If devices entity is invalid, FALSE will be returned.
 */
aud_bool_t
dr_devices_get_fire_callbacks_on_device_close
(
	dr_devices_t * devices
);


// Opt-in for new behaviours

/**
 * Enable TX channel names being associated with TX channels and not separate labels
 *
 * @param devices [IN] the devices object
 *
 * Once enabled, all TX channel naming operations will apply through TX channels,
 * not TX labels.  No TXLABEL status events will fire, and the results of calling
 * any txlabel function are undefined.
 *
 * Defaults to disabled for legacy API support.
 *
 * @note should call this before creating any devices
 */
void
dr_devices_enable_txchannel_names
(
	dr_devices_t * devices
);
 ...
 *
 * @note in media mode, returns audio channels only. Use dr_device_get_media_txchannels() for
 * non-audio channels
 */
aud_error_t
 ...
 *     an error code otherwise.
 *
 * @note in media mode, returns audio channels only.  Use dr_device_get_media_rxchannels() for
 * non-audio channels
 */
aud_error_t
...
/**
 * Forcefully mark all changeable components as stale. Subsequent
 * component updates will refresh all components' state.
 *
 * @param device the device object
 */
void
dr_device_mark_all_components_stale
(
	dr_device_t * device
);
 ...
 * @param request_id a pointer to a value that will be the request id for this operation.
 *
...
/**
 * Are multicast performance settings configurable for this device?
 *
 * This function only shows device capability whether multicast performance can be set.
 * This function does not show whether a device can be configured in a domain.
 * The user should check the access policy when the device is in a domain.
 *
 * @param device the device
 */
aud_bool_t
dr_device_can_set_tx_performance
(
	const dr_device_t * device
);
...
/**
 * Set the device's TX audio delay
 *
 * @param device the device object
 * @note delay should not be greater than the maximum allowed
 */
aud_error_t
dr_device_set_tx_audio_delay
(
	dr_device_t * device,
	dr_device_response_fn * response_fn,
	dante_request_id_t * request_id,
...
/**
 * Get this transmit channel's id
 *
 * @param tx the transmit channel
 */
dante_id_t
...
/**
 * Get the current name of the given TX channel.
 *
 * @param tx the transmit channel
 *
 ...
 */
const char *
dr_txchannel_get_name
(
	const dr_txchannel_t * tx
);

/**
 * Get the default name of the given TX channel.
 * A channel's default name is fixed and will not change.
 *
 * @param tx the transmit channel
 *
 * @returns the name of the given channel
 */
const char *
dr_txchannel_get_default_name
(
	const dr_txchannel_t * tx
);

/**
 * Get the video format information for this channel.
 *
 * @param tx the transmit channel
 */
const dante_video_format_t *
dr_txchannel_get_video_format
(
	const dr_txchannel_t * tx
);
...
/**
 * Set the name of a TX channel
 *
 * @param tx the transmit channel to rename
 * @param response_fn the function to call upon completion of this operation
 * @param request_id the id for this operation
 * @param new_name the new_name for the transmit channel
 *     (or NULL to set to the default name)
 *
 * @return AUD_SUCCESS if the channel rename was successfully initiated,
 *     an error code otherwise.
 *
 * New name must be unique on device (per media type).
 * - Setting a TX channel's name to another TX channel's default name will fail.
 * - Otherwise, if another channel is already using the new name then the name
 *   will be moved to the current channel, and the other channel will revert to
 *   its default name.
 */
aud_error_t
dr_txchannel_set_name
(
	dr_txchannel_t * tx,
	dr_device_response_fn * response_fn,
	dante_request_id_t * request_id,
...
/**
 * Get the status of the external interface to the transmit channel
 *
 * @param txc the transmit channel
 */
dante_extstatus_t
dr_txchannel_get_extstatus
(
	const dr_txchannel_t * txc
);

/**
 * Get the read-only state of the transmit channel.
 *
 * @param tx the transmit channel
 *
 * @returns AUD_TRUE if the channel is read-only, AUD_FALSE otherwise
 */
aud_bool_t
dr_txchannel_is_read_only
(
	dr_txchannel_t * tx
);
...
/**
 * Get the default name of the given RX channel.
 * A channel's default name is fixed and will not change.
 *
 * @param rx the receive channel
 *
 * @returns the name of the given channel (or NULL if unavailable)
 *
 * @note returns NULL on audio-only devices
 */
const char *
dr_rxchannel_get_default_name
(
	const dr_rxchannel_t * rx
);
 ...
 *     (or NULL to set it to the default name)
 *
 * @return AUD_SUCCESS if the channel rename was successfully initiated,
 *     an error code otherwise.
 */
...
/**
 * Get the status of the external interface to the receive channel
 *
 * @param rxc the receive channel
 */
dante_extstatus_t
dr_rxchannel_get_extstatus
(
	const dr_rxchannel_t * rxc
);

/**
 * Get the processing status to the receive channel
 *
 * @param rxc the receive channel
 */
dante_media_processing_status_t
dr_rxchannel_get_media_processing_status
(
	const dr_rxchannel_t * rxc
);

/**
 * Get the read-only state of the receive channel.
 *
 * @param rx the receive channel
 *
 * @returns AUD_TRUE if the channel is read-only, AUD_FALSE otherwise
...
/**
 * Get the number of audio flows currently being transmitted by this
 * device
 *
 * @param device the device object
 */
...
/**
 * Get the number of media flows currently being transmitted by this
 * device
 *
 * @param device the device object
 * @param media_type the flow's media type
 *
 * @note media_type DANTE_MEDIA__ALL will return all flow count
 */
uint16_t
dr_device_num_media_txflows
(
	dr_device_t * device,
 ...
 * (ie. it is transmitting audio). Each bit represents one device interface; a bit is set only
 * if the flow has an address on that interface AND it is transmitting audio on that interface.
 *
 * @param flow the flow handle
 ...
 */
aud_error_t
dr_txflow_get_connections_active
(
...
/**
 * Get the flow data status of a transmit flow on a given interface.
 * This only applies for encrypted configured TX flows.
 *
 * @param flow the flow handle
 * @param interface_index the interface index to use for resolution information
 * @param data_status on success, a pointer to a location where to store flow data status
 */
aud_error_t
dr_txflow_get_flow_data_status
(
	const dr_txflow_t * flow,
	uint16_t interface_index,
	dante_flow_data_status_t * data_status
);
...
/**
 * Get the read-only state of the transmit flow.
 *
 * @param flow the transmit flow
 *
 * @returns AUD_TRUE if the flow is read-only, AUD_FALSE otherwise
 */
aud_bool_t
dr_txflow_is_read_only
(
	dr_txflow_t * flow
);
...
/**
 * Get the number of audio flows currently being received by this
 * device
 *
 * @param device the device object
 */
...
/**
 * Get the number of media flows currently being received by this
 * device
 *
 * @param device the device object
 ...
 * @param flow_ptr a pointer to a flow handle
 *
 * @note in media mode, returns audio flows only. Use dr_device_media_rxflow_at_index() for
 * non-audio flows
 */
aud_error_t
...
/**
 * Get the format for a given video RX flow
 *
 * @param flow the flow handle
 * @param type a pointer to a location to store the flow's video format
 */
const dante_video_format_t*
dr_rxflow_get_video_format
(
	dr_rxflow_t* flow
);

/**
 * Get the format for a given ancillary RX flow
 *
 * @param flow the flow handle
 * @param type a pointer to a location to store the flow's ancillary format's type
 */
aud_error_t
dr_rxflow_get_ancil_format
(
	dr_rxflow_t * flow,
	uint32_t * type
);
...
/**
 * Get the flow data status of a receive flow on a given interface.
 * This only applies for encrypted configured RX flows.
 *
 * @param flow the flow handle
 * @param interface_index the interface index to use for resolution information
 * @param data_status on success, a pointer to a location where to store flow data status
 */
aud_error_t
dr_rxflow_get_flow_data_status
(
...
/**
 * Get the maximum display group ID that this device supports.
 *
 * @param device the device object
 *
 * @return the max display group ID,
     or zero (0) if display groups are not supported
 */
dante_id_t
dr_device_max_display_group_id
(
	const dr_device_t * device
);

/**
 * Return whether a display group is stale or not
 *
 * @param device the device object
 * @param dg_id  the identifier of the display group
 */
aud_bool_t
dr_display_group_is_stale
(
	dr_device_t * device,
	dante_id_t dg_id
);
...
/**
 * Get a display group's name
 *
 * @param device the device object
 * @param dg_id  the identifier of the display group
 *
 * @return the display group name, or NULL if the label does not exist or on error
 */
const char *
dr_display_group_name_for_id
(
	dr_device_t * device,
	dante_id_t dg_id
);
...
/**
 * Get this transmit channel's display group id
 *
 * @param tx the transmit channel
 *
 * @return the TX channel's display group ID,
     or zero (0) when TX channel handle is invalid
 */
dante_id_t
dr_txchannel_get_display_group_id
(
	const dr_txchannel_t * tx
);

/**
 * Get this transmit channel's display group name
 *
 * As dr_display_group_name_for_id(), with dr_txchannel_get_display_group_id().
 *
 * @param tx the transmit channel
 *
 * @return the TX channel's display group name,
     or NULL if no name is available
 */
const char *
dr_txchannel_get_display_group_name
(
	const dr_txchannel_t * tx
);

/**
 * Get this receive channel's display group id
 *
 * @param rx the receive channel
 *
 * @return the RX channel's display group ID,
     or zero (0) when RX channel handle is invalid
 */
dante_id_t
dr_rxchannel_get_display_group_id
(
	const dr_rxchannel_t * rx
);

/**
 * Get this receive channel's display group name
 *
 * As dr_display_group_name_for_id(), with dr_rxchannel_get_display_group_id().
 *
 * @param rx the receive channel
 *
 * @return the RX channel's display group name,
     or NULL if no name is available
 */
const char *
dr_rxchannel_get_display_group_name
(
	const dr_rxchannel_t * rx
);
