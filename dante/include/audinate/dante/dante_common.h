/*
 * Copyright © 2009-2025 Audinate Pty Ltd ACN 120 828 006 (Audinate). All rights reserved.
 *
 *
 ...
 *
 * @return the version as a uint32
 */
AUD_INLINE uint32_t
...
enum dante_capability
{
	...
	DANTE_CAPABILITY_CLEAR_CONFIG,
	DANTE_CAPABILITY_INTERFACE_CLEAR_ERROR_COUNTER,
	...
	DANTE_CAPABILITY_ENROL,
	DANTE_CAPABILITY_COUNT,

	// @deprecated Use DANTE_CAPABILITY_CLEAR_CONFIG instead
	DANTE_CAPABILITY_CLEAR_CONIFG = DANTE_CAPABILITY_CLEAR_CONFIG,

	// @deprecated Use DANTE_CAPABILITY_INTERFACE_CLEAR_ERROR_COUNTER instead
	DANTE_CAPABILITY_INTERACFE_CLEAR_ERROR_COUNTER = DANTE_CAPABILITY_INTERFACE_CLEAR_ERROR_COUNTER,
};
...
enum dante_data_status
{
	DANTE_INTERFACE_DATA_STATUS_NONE,
	// data status just initialised or reset upon flow status not being active

	DANTE_INTERFACE_DATA_STATUS_ACTIVE,
	// data is being transferred/received

	DANTE_INTERFACE_DATA_STATUS_ENCRYPT_PENDING,
	// information necessary for data encryption/decryption has not been received yet

	DANTE_INTERFACE_DATA_STATUS_SESSION_KEY_ID_MISMATCH,
	// session key ID in encrypted packet doesn’t match key ID in flow context

	DANTE_INTERFACE_DATA_STATUS_FLOW_ID_MISMATCH,
	// flow ID in packet doesn't match flow ID in flow context

	DANTE_INTERFACE_DATA_STATUS_SENTINEL_CHECK_FAILED,
	// the sentinel check failed

	DANTE_INTERFACE_DATA_STATUS_ENCRYPT_SUBSYSTEM_ERROR,
	// the (typically external) library function used to encrypt/decrypt data returned an error

	DANTE_INTERFACE_DATA_STATUS_INVALID_DATA,
	// the packet data was invalid

	DANTE_INTERFACE_DATA_STATUS_COUNT
};

	/** A type for data status code enumerations */
typedef uint16_t dante_flow_data_status_t;
...
enum dante_rxstatus
{
	...
	DANTE_RXSTATUS_SUBSCRIBE_SELF_POLICY = 0x22,
	  ...
	  /** Discovered TX channel is not ready */
	...
	DANTE_RXSTATUS_HDCP_NEGOTIATION_ERROR = 0x70,
	  ...
	  /** Error: TX requires encryption but RX does not support any of the requested encryption schemes */
	DANTE_RXSTATUS_RX_ENCRYPTION_UNSUPPORTED = 0x71,

	  /** Error: TX requires a transport protocol but RX does not support it */
	DANTE_RXSTATUS_RX_TRANSPORT_UNSUPPORTED = 0x72,
...
enum dante_flow_class
{
	...
	DANTE_FLOW_CLASS__AES67_IP,
	DANTE_FLOW_CLASS__DANTE_RTP_VIDEO_IP,
	DANTE_FLOW_CLASS__DANTE_V3_IP,

	DANTE_FLOW_CLASS_COUNT,

	// deprecated values
	DANTE_FLOW_CLASS__AES67_MCAST_IP = DANTE_FLOW_CLASS__AES67_IP,
	DANTE_FLOW_CLASS__AES67_UCAST_IP = DANTE_FLOW_CLASS__AES67_IP,
};
...
AUD_INLINE aud_bool_t
dante_flow_class_is_dante_audio(dante_flow_class_t flow_class)
{
	switch (flow_class)
	{
		case DANTE_FLOW_CLASS__DANTE_IP:
		case DANTE_FLOW_CLASS__DANTE_V3_IP:
			return AUD_TRUE;
		default:
			return AUD_FALSE;
	}
...
/**
 * Helper method to validate whether a socket (file descriptor) can legally be added to
 * a fd_set (and thus to a dante_sockets structure)
 *
 * @param socket socket to validate.  Note that this is actually just an int, and does not
 *	need to be otherwise initialised.
 * @returns 0 if invalid, non-zero if valid
 *
 */
AUD_INLINE int
dante_sockets_is_valid_fd(aud_socket_t s)
{
#ifdef WIN32
    return (s != AUD_SOCKET_INVALID);
#else
    if (s >= (aud_socket_t)FD_SETSIZE)
    {
        return 0;
    }
    return (s >= 0);
...
{
	if (! dante_sockets_is_valid_fd(s))
	{
		return;
	}
...
/**
 * Get a string for the given flow data status value
 *
 * @param status the status
 */
const char *
dante_flow_data_status_to_string
(
	dante_flow_data_status_t status
);
...
const char *
dante_flow_class_to_string(dante_flow_class_t);


//----------------------------------------------------------
// Dante transport protocols
//----------------------------------------------------------
typedef enum dante_transport
{
	DANTE_TRANSPORT__NONE = 0,

	DANTE_TRANSPORT__ATP = 1,
		//!< ATPv2

	DANTE_TRANSPORT__ATPV3 = 2,
		//!< ATPv3

	DANTE_TRANSPORT__RTP = 3,
		//!< RTP

	DANTE_TRANSPORT__INVALID = 0xff
		//!< Invalid transport protocol
} dante_transport_t;

enum
{
	DANTE_TRANSPORT_FLAG__ATP             = (1 << (DANTE_TRANSPORT__ATP - 1)),
	DANTE_TRANSPORT_FLAG__ATPV3           = (1 << (DANTE_TRANSPORT__ATPV3 - 1)),
	DANTE_TRANSPORT_FLAG__RTP             = (1 << (DANTE_TRANSPORT__RTP - 1)),
};

typedef uint16_t dante_media_transport_t;
