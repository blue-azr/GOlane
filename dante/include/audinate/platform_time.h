 ...
 *
 * This software is Copyright (c) 2004-2021, Audinate Pty Ltd and/or its licensors
 *
...
/**
 * Get the current monotonic time for the local machine in seconds/microseconds.
 *
 * If monotonic time is not available, return the best available approximation.
 * Use this function for getting timestamps for application timing.
 *
 * On most platforms, this wraps aud_utime_get_monotonic().
 *
 ...
 */
aud_error_t
aud_utime_get_for_timing
(
	aud_utime_t * at
);

/**
 * Get the current wall-clock time for the local machine in seconds/microseconds.
 *
 * If wall-clock time is not available, return the best available approximation.
 * Use this function for getting timestamps for logging to file or other reporting.
 *
 * On most platforms, this wraps aud_utime_get_wallclock().
 *
 * @param at the time structure that will contain the current time
 *
 * @return AUD_SUCCESS if the current time was successfully obtained, an error otherwise
 */
aud_error_t
aud_utime_get_for_reporting
(
	aud_utime_t * at
);

/**
 * Get the current system time for the local machine in seconds/microseconds.
 *
 * Historically, this was equivalent to the POSIX gettimeofday() function.
 * Currently, it usually wraps aud_utime_get_for_timing()
 *
 * @deprecated Explicitly use aud_utime_get_for_timing() or aud_utime_for_reporting() depending on what
 * sort of time is required..
 *
 * @param at the time structure that will contain the current time
 *
 * @return AUD_SUCCESS if the current time was successfully obtained, an error otherwise
...
{
	return aud_utime_get_for_timing(at);
}
...
/**
 * @deprecated use aud_utime_get_for_timing or aud_utime_get_for_reporting instead
 ...
 *
 * @note Passing NULL for the time will use the current time from aud_utime_get_for_reporting.
 * 
